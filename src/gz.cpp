...
auto gz::BitStreamMixin::push_footer(std::uint32_t crc_on_uncompressed, std::uint32_t num_bytes_uncompressed) -> void {
    push(crc_on_uncompressed);
    push(num_bytes_uncompressed);
}

gz::BitStream::BitStream(std::ostream& stream) : bits_(0), num_bits_(0), out_(stream) {}

gz::BitStream::~BitStream() {
    flush_byte();
}
...
gz::BufferedBitStream::BufferedBitStream(BitStream& bit_stream) : bits_(0), num_bits_(0), wrapped_(bit_stream) {}
...
auto gz::BufferedBitStream::bits() const -> std::size_t {
    return (buffer_.size() * size_of_in_bits<decltype(buffer_)::value_type>()) + num_bits_;
}
...
auto gz::BufferedBitStream::unbuffered() -> BitStream& {
    return wrapped_;
}
