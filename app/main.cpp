...
struct BlockStreamWithBreakpoint {
  ...
  std::size_t breakpoint; // The maximum number of uncompressed bytes that can
                          // be stored in a block in the stream
};
...
auto main() -> int {
  ...
  char next_byte{}; // Note that we have to use a (signed) char here for
                    // compatibility with istream::get()
  ...
  const auto block_streams_with_breakpoints =
      std::array<BlockStreamWithBreakpoint, 3>{
          BlockStreamWithBreakpoint{
              .block_stream = std::make_unique<
                  BlockType0Stream<maximum_block_type_0_capacity>>(stream),
              .breakpoint = maximum_block_type_0_capacity},
  ...
  auto commit_smallest = [&block_streams_with_breakpoints,
                          &uncompressed_block_size](bool is_last) {
    ...
    for (auto i = 0; i < block_streams_with_breakpoints.size(); ++i) {
      if (uncompressed_block_size >
          block_streams_with_breakpoints.at(i).breakpoint) {
        continue;
      }
      const auto compressed_block_size =
          block_streams_with_breakpoints.at(i).block_stream->bits(is_last);
    ...
    block_streams_with_breakpoints.at(smallest_compressed_block_type)
  ...
  if (std::cin.get(next_byte)) {
    ...
    while (1) {
      ...
      crc = CRC::Calculate(&next_byte, 1, crc_table,
                           crc); // Add the character we just read to the CRC
                                 // (even though it is not in a block yet)
